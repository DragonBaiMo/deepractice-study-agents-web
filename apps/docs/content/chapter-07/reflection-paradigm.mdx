...
如图7.3所示，这个循环可以重复进行多次，直到反思阶段不再发现新的问题，或者达到预设的迭代次数上限。我们可以将这个迭代优化的过程形式化地表达出来。假设 $O_i$ 是第 $i$ 次迭代产生的输出（$O_0$ 为初始输出），反思模型 $\pi_{\text{reflect}}$ 会生成针对 $O_i$ 的反馈 $F_i$：
...
<img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-3.png" alt="Reflection机制中的执行-反思-优化迭代循环" className="w-[70%]" />
</div>
<p className="text-center text-sm text-gray-500 mt-2">图 7.3 Reflection 机制中的"执行-反思-优化"迭代循环</p>
...
[⬅️ 上一节：Plan-and-Solve范式](plan-and-solve-paradigm) | [🏠 返回目录](README.md) | [➡️ 下一节：从手写到框架](paradigm-comparison)
