...
在前面的章节中，我们学习了 PromptX（第四章）、AgentX（第五章）、Deepractice 方法论（第六章）以及智能体经典范式（第七章）。
...
第四章提出的"上下文鸿沟"问题：AI 缺乏领域知识、角色意识和记忆能力。
